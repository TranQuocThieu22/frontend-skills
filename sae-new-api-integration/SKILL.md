...
13. **Date Payload Formatting (MANDATORY - UTC Standard)**: Khi xử lý payload của các form có chứa ngày bắt đầu và ngày kết thúc (ví dụ: `startDate`, `endDate`, `openRegisterDate`, `closeRegisterDate`), bạn BẮT BUỘC phải đưa về chuẩn thời gian quốc tế (UTC) và format thêm giờ phút giây cụ thể. Tuyệt đối KHÔNG ĐƯỢC dùng `.format("YYYY-MM-DDT00:00:00")` vì nó là "Fake UTC" gây sai lệch múi giờ. Hãy sử dụng `dayjs` kết hợp `.toISOString()` để thư viện tự động chuyển đổi từ giờ địa phương sang UTC chuẩn. Ví dụ: `dayjs(body.startDate).startOf('day').toISOString()` và `dayjs(body.endDate).endOf('day').toISOString()`.
