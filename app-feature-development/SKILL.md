...
- **No Modal-Button Separation (Unified Trigger Components)**: Do NOT separate a trigger Button/Icon and its corresponding Modal/Drawer into separate files or separate component exports.
  - **Unified Integration**: Always use unified components (like `CustomModalTrigger`, `<CustomButtonCreateUpdate>`, or your own unified feature components such as `CommunityCriteriaCreateUpdate`) that house both the button trigger and the modal internally within a single file.
  - **CustomModalTrigger Best Practices**: `CustomModalTrigger` manages its own internal open/close state via the `useDisclosure` hook. You do NOT need to pass a `disclosure` prop.
    - **Trigger Prop**: Use the `trigger` render prop to dynamically render the button. This allows the component to be used both in toolbars (as a Button) and inside table row actions (as an ActionIcon). Example: `trigger={(open) => isUpdate ? <CustomActionIcon onClick={open}><IconEdit /></CustomActionIcon> : <CustomButton onClick={open}>Thêm</CustomButton>}`.
    - **Children Prop**: Use the `children` render prop to access the `close` function so you can manually close the modal upon form submission or cancellation. Example: `{(close) => <Box><Form/><CustomButton onClick={close}>Đóng</CustomButton></Box>}`.
  - **Table Render Support**: Use these unified components even when they are rendered inside a table (e.g., as row actions inside a custom table cell or toolbar). By passing an `isUpdate` prop, the same component can render a different trigger UI.
...
- **Extract Action Buttons (Mandatory)**: Action buttons (especially those containing complex mutation logic, API calls like Delete, Start, or Approve, or those using `CustomModalTrigger`) inside tables or layout components MUST be extracted into their own separate component files (e.g., `ActivityListDeleteButton.tsx`, `AttendanceDetailCheckInButton.tsx`) and imported. Do NOT write API mutation logic, `CustomModalTrigger` wrappers, or complex action components inline inside the main table component (`*Table.tsx` or `*Modal.tsx`) to keep the parent file clean, maintainable, and avoid React Hook errors in render functions.
...
- **Prototype Menu Flags (`isPrototype`)**: Whenever the user instructs to "implement only the UI" or "develop the UI first" ("làm trước UI"), you **MUST ALWAYS** add the new route to the corresponding module's layout menu (`src/app/.../layout.tsx`) and append the property `isPrototype: true` to the menu item. **CRITICAL**: Even if the page is technically a sub-page or a hidden child route (like a detail page), you must temporarily add it as a visible menu item in the sidebar during the prototype phase so the team can easily click and preview the UI without manually typing the URL.
...
- **Read-Only Entity Code on Update (CRITICAL)**: When designing a Create/Update form, the primary entity code (e.g., `Mã kế hoạch`, `Mã nhóm`, `Mã danh mục`) MUST be locked (set to `readOnly={true}` or `disabled={true}`) when the form is in `isUpdate` mode. Entity codes are usually primary keys or unique identifiers and must not be mutated after creation.
- **Flatten Sidebar URLs (1-Level Routes) (CRITICAL)**: Avoid creating multi-level nested URL structures for sidebar links (e.g., do NOT use `/operation/support-tool/attendance/reconciliation`). Instead, flatten the routes to exactly one level under the main module path (e.g., `/operation/support-tool/attendance-reconciliation`). This structure eliminates prefix overlapping conflicts inside the `CustomAppShell` navigation sidebar component and ensures the layout UI highlights the menu correctly without nesting logic issues.
- **Layout Architecture & Atomic Queries (No Global Queries)**: Keep Layout components thin and move state and query logic into independent, encapsulated sub-components.
  ...
  - **No Hardcoded Wrappers / Redundant Padding (CRITICAL)**: Layout components MUST NOT wrap their contents in unnecessary `<Box>` or `<div>` containers with hardcoded styling (e.g., `height: "100vh"`, `padding: 16`, `p="md"`, or `overflowY: "auto"`). The global `CustomAppShell` already provides default structural padding and scroll behavior. Never wrap your UI just to add your own padding. Use `<Stack>` (with appropriate gap as needed) or `<Grid>` to organize the layout children directly.
...
- **Avoid Redundant Page Headers (CRITICAL)**: When implementing ANY page UI (whether it uses a `CustomTanstackTable`, a Grid layout, or complex forms), do **NOT** create a separate `...Header.tsx` component and do **NOT** manually add page titles (`<Title>`), back buttons (`IconArrowLeft`), or `<Breadcrumbs>`. The global `CustomAppShell` automatically renders the breadcrumb and page title based on the active routing menu. Keep the Layout component thin, and rely on the internal section headings (e.g. Table titles or Card headers) for context.
...
- **CustomTanstackTable Implementation (CRITICAL)**: Do NOT use standard `@tanstack/react-table` API properties directly in the `columns` array.
  - You **MUST** use `CustomTanstackTableColumnDef` format (which expects a capital `Cell` instead of `cell` and requires an `accessorKey` for every column).
  - Do NOT manually define an `actions` or `Thao tác` column inside your `columns` array. Instead, pass your action buttons directly into the `renderRowActions` prop of `CustomTanstackTable`. The table component natively injects and manages the action column safely. Doing this manually will trigger `Unhandled Runtime Error: Columns require an id when using a non-string header`.
...
3. **Do NOT add a `web.config`**: With `output: 'export'` + `trailingSlash: true` and no dynamic route segments, every route exports to a physical `<route>/index.html` that IIS serves through its default document — no rewrite rule is needed, and refresh/deep-link already works. Only add `/public/web.config` when the app needs something IIS cannot do by default (e.g. `osp` reverse-proxies `/api` because its backend has no CORS). If you do add one, write rules **relative** (`match "^api/(.*)"`, `rewrite "index.html"`) — never hardcode an alias like `/srm/index.html`, since Preview deploys under `/pr-<id>/<app>`. The pipeline no longer generates or overwrites this file; the repo copy is the only source of truth.
