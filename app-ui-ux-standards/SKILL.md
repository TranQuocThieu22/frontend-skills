...
15. **Use readOnly instead of disabled for Business Logic Read-Only Fields**: When a form field needs to be uneditable due to business logic or a "view-only" mode (e.g., viewing details), ALWAYS use the `readOnly` prop instead of `disabled`. Using `disabled` will gray out the field and make the text harder to read, whereas `readOnly` prevents editing while keeping the text clear and readable.
    - **Applies to `CustomSelect` too (frequently missed)**: the dashed-border / italic "read-only" styling is rendered ONLY when `readOnly` is passed (`styles.input` in `CustomSelect` branches on `rest.readOnly`). Passing `disabled={isViewMode}` produces Mantine's default gray disabled look, which is visually inconsistent next to `CustomTextInput` / `CustomDateInput` / `CustomNumberInput` in the same form.
    - Pair it with `clearable={!isViewMode}` so the clear (X) button disappears in view mode.
    - **`CustomFileInput` and `CustomCheckbox` support `readOnly` too**: both were extended to render the same dashed-border read-only look as `ModernInput.module.css` / `threeDTheme`. `CustomFileInput` also hides its clear button when read-only; `CustomCheckbox` blocks interaction via `pointerEvents: 'none'` on the root (the native `readonly` attribute does NOT stop `input[type=checkbox]` from toggling) and keeps the checked background so the tick stays visible.
    - Recommended pattern: one `const isView = mode === "view";` flag, then `readOnly={isView}` on EVERY field (text/number/date/select/file/checkbox) and hide the Save button entirely. There is no reason to reach for `disabled` in a view-only form.
