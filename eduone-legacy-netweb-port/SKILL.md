---
name: eduone-legacy-netweb-port
description: Quy trình port một màn hình từ dự án Angular netweb cũ sang app EduOne (Next.js + Mantine) - tìm code nguồn, dùng đúng API legacy, dựng bảng/form theo core-ui và quy ước đặt bộ lọc.
---

# Port màn hình từ netweb (Angular) sang EduOne (Next.js)

Dùng khi có yêu cầu dạng "kiểm tra code màn hình X bên netweb cũ và tích hợp cho EduOne".
...
Quy ước gọi API trong EduOne (`src/features/<feature>/api/<ten>.api.ts`):
...
- Dev mode đi qua proxy khai báo ở `apps/eduone/src/middleware.ts` (matcher `/api/:path*`), không có rewrite trong `next.config.ts`.
...
## 4. Cấu trúc feature trong EduOne
...
Bản cũ dùng `loai_thaotac`: `1 = thêm`, `2 = xem`, `3 = sửa`. Trong EduOne đổi thành prop `mode: 'create' | 'update' | 'view'`:
...
- Chạy `npx tsc --noEmit -p tsconfig.json` trong `apps/eduone` và lọc lỗi theo tên feature — repo còn nhiều lỗi type cũ ở feature khác, đừng nhận nhầm.
