...
- **Avoid Redundant Page Headers (CRITICAL)**: When implementing a page UI (especially during Prototype UI phase) that consists primarily of a `CustomTanstackTable`, do **NOT** create a separate `...Header.tsx` component and do **NOT** manually add page titles (`<Title>`), back buttons, or `<Breadcrumbs>`. The global `CustomAppShell` automatically renders the breadcrumb/title based on the menu, and the Table's own `title` prop already provides sufficient context. Keep the Layout component thin (just `<Stack>` and `<Table>`).
