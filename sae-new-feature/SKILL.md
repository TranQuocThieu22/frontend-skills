...
- **No Modal-Button Separation (Unified Trigger Components)**: Do NOT separate a trigger Button/Icon and its corresponding Modal/Drawer into separate files or separate component exports.
  ...
  - **CustomButtonModal Usage (CRITICAL)**: When using `CustomButtonModal` from `@aq-fe/core-ui`, you MUST be aware that it does NOT automatically handle form footers or provide props like `onConfirm`, `buttonLabel`, or `confirmButtonLabel`.
    - You MUST initialize a disclosure hook (`const disc = useDisclosure(false);`) and pass it to the component: `<CustomButtonModal disclosure={disc} ...>`.
    - You MUST manually render the modal body and the action footer (e.g., Save/Cancel buttons) as `children` of `CustomButtonModal`.
    - Failure to pass `disclosure` will cause a fatal `TypeError: Cannot read properties of undefined (reading '0')` when the modal tries to open.
